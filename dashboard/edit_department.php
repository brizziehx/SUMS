<?php
    ...
    if(isset($_POST['update'])) {
        ...
        if(empty($department)) {
            ...
            if(!preg_match("/^[a-zA-Z ]{2,}$/", $department)) {
                ...
                $stmt = $conn->prepare("SELECT * FROM department WHERE deptName = :deptName AND deptID <> :deptID");
        ...
        if(!array_filter($errors)) {
            $stmt = $conn->prepare("UPDATE department SET deptName = :deptName WHERE deptID = :deptID");
            $stmt->bindValue(':deptName', $department, PDO::PARAM_STR);
            $stmt->bindValue(':deptID', $_REQUEST['id'], PDO::PARAM_INT);
            if($stmt->execute()) {
                $department = "";
                $success['suc'] = "Department has been updated Successfully";
            }
        }
    }
    ...
    $stmt = $conn->prepare("SELECT * FROM department WHERE deptID = :deptID");
    $stmt->bindValue(':deptID', $_REQUEST['id'], PDO::PARAM_INT);
    $stmt->execute();
...
<head>
    ...
    <title>Update Department | UMS</title>
...
<body>
    ...
    <aside>
        ...
        <main>
            <header>
            <h3 class="bread-cumb"><a href="index.php">Dashboard</a> > <a href="departments.php">Departments</a> > Edit Department</h3>
                ...
                <div class="user-add" style="grid-column: span 5">
                    ...
                    <h3>Edit Department</h3>
                        <div class="input">
                            ...
                            <input type="text" name="department" value="<?=$row2[1]?>">
