...
<body>
    ...
    <div class="container">
        ...
        <main>
            <header>
                <h3 class="bread-cumb"><a href="index.php">Dashboard</a> > <a href="all_pending.php">Pending Uniforms</a> > All Pending Dimensions</h3>
                    ...
                    <form>
                        ...
                        <?php
                            ...
                            while($row = $usersDep->fetch(PDO::FETCH_ASSOC)):
                                $uniform = $conn->query("SELECT uniform.*, dimensionS.* FROM dimensionS INNER JOIN uniform ON uniform.unID = dimensions.uniformID WHERE (year(app_date_time) = year(current_date()) AND dimensions.userID = {$row['userID']}) AND dimensions.status = 'pending'");
