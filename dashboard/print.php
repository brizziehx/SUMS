...
$dims = $conn->query("SELECT dimensions.*, uniform.* FROM dimensions INNER JOIN uniform ON dimensions.uniformID = uniform.unID WHERE year(app_date_time) = year(current_date())  AND userID = {$_REQUEST['id']}");
$comp = $conn->query("SELECT dimensions.*, uniform.* FROM dimensions INNER JOIN uniform ON dimensions.uniformID = uniform.unID WHERE (year(app_date_time) = year(current_date()) AND dimensions.sup_status = 'complete') AND userID = {$_REQUEST['id']}");

$dimRow = $dims->fetch_assoc();
    ...
    <div class="container-r">
        <div class="header-content print">
            ...
            <div class="details">
                ...
                <div class="row">
                    <b>Month: </b> <?=date('F Y', strtotime($dimRow['app_date_time']))?>
                </div>
            </div>
            <div class="content">
                <h3>Ordered Uniform</h3>
                <table>
                    <tr>
