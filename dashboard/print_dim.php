...
$dims = $conn->query("SELECT dimensions.*, uniform.* FROM dimensions INNER JOIN uniform ON dimensions.uniformID = uniform.unID WHERE year(app_date_time) = year(current_date())  AND userID = {$_REQUEST['userID']}");

$dimRow = $dims->fetch_assoc();

$sql = $conn->query("SELECT USER.*, department.* FROM user INNER JOIN department ON user.deptID = department.deptID WHERE userID = {$_REQUEST['userID']}");
